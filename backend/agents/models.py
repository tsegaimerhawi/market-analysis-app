...
class MacroOutput(BaseModel):
    ...
    stance: float = Field(
        ge=-1.0, le=1.0, description="-1 (bearish) to 1 (bullish) for risk assets"
    )
    summary: str = Field(default="", max_length=500)
    confidence: float = Field(default=0.5, ge=0.0, le=1.0)
...
class TradeDecision(BaseModel):
    ...
    guardrail_triggered: bool = Field(
        default=False, description="True if hard guardrail overrode the AI"
    )
    weights_used: Optional[dict] = (
        None  # e.g. {"lstm": 0.4, "xgboost": 0.2, "sentiment": 0.3, "macro": 0.1}
    )
