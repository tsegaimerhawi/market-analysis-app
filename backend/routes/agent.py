...
def _volatile_candidates_path():
    return os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "volatile_symbols.json"
    )
...
def _volatile_universe_path():
    return os.path.join(
        ...
        "data",
        "volatile_universe.json",
    )


def _normal_candidates_path():
    return os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "normal_symbols.json"
    )
...
def api_volatile_candidates_refresh():
    try:
        ...
        return jsonify(
            {
                "symbols": symbols,
                "message": f"Updated from universe (top {len(symbols)} by volatility)",
            }
        )
    except Exception as e:
...
def api_agent_telegram_test():
    try:
        ...
        return jsonify(
            {"ok": False, "error": err or "Set TELEGRAM_HTTP_API_KEY and TELEGRAM_CHAT_ID."}
        )
    except Exception as e:
