...
class EnsemblePredictor:
    ...
    def predict_recursive(self, model, last_window, steps, n_lags=5):
        ...
        for _ in range(steps):
            ...
            if len(current_window) >= 2:
                ret = (
                    (current_window[-1] - current_window[-2]) / current_window[-2]
    ...
    def run_ensemble(self, data_config, source, steps, selected_algos=None):
        ...
        if df is None:
            return {
                "error": f"Failed to load data for {source}. Please check the symbol and date range."
            }
            ...
            try:
                if algo_id == "lstm":
                    ...
                    if X is not None and len(X) >= 20:
                        ...
                        preds = predict_future_steps(
                            model, last_prices, steps, lstm_seq_len, min_val=min_v, range_val=rng_v
                        )
                ...
                if model:
                    ...
                    if X is not None:
                        model.fit(X, y)
                        preds = self.predict_recursive(
                            model, df["Close"].values[-n_lags:], steps, n_lags
                        )
                        results[algo_id] = [float(p) for p in preds]
                        all_preds.append(results[algo_id])
        ...
        if not all_preds:
            return {
                "error": f"Insufficient data to train models for {source}. Try a longer date range."
            }
