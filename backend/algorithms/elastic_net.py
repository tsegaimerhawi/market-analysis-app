...
def run_algorithm(data_config, source):
    ...
    if df is None:
        return result_dict("Elastic Net", {}, None, None, None, error="Failed to load or filter data")
    X, y, idx = build_lag_features(df["Close"], n_lags=5)
    if X is None:
        return result_dict("Elastic Net", {}, None, None, None, error="Insufficient data for features")
